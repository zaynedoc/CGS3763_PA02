		...
		while (true) {
			...
			if (cmd.equals("EXIT")) {
				...
				try {
					...
					for (int i = 0; i < Banker.NUMBER_OF_RESOURCES; i++) req[i] = Integer.parseInt(parts[2 + i]);
				...
				try {
					...
					for (int i = 0; i < Banker.NUMBER_OF_RESOURCES; i++) rel[i] = Integer.parseInt(parts[2 + i]);
